...
<div class="px-5 pt-6 pb-10">
  ...
  <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
    <p class="mb-4 text-sm font-medium text-slate-700">Enter your clean date</p>

    <div class="grid grid-cols-3 gap-3">
      <div>
        <label for="month" class="mb-1 block text-xs text-slate-500">Month</label>
        <select id="month" bind:value={selectedMonth} class="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-900">
      ...
      <div>
        <label for="day" class="mb-1 block text-xs text-slate-500">Day</label>
        <select id="day" bind:value={selectedDay} class="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-900">
      ...
      <div>
        <label for="year" class="mb-1 block text-xs text-slate-500">Year</label>
        <select id="year" bind:value={selectedYear} class="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-900">
          {#each years as y (y)}
  ...
  <!-- Results -->
  {#if result}
    <div class="mt-5 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
      {#each formatResult(result).split('\n') as line, i (i)}
        {#if i === 0}
          <p class="text-center text-lg font-bold text-slate-900">{line}</p>
        {:else}
          <p class="mt-1 text-center text-sm text-slate-600">{line}</p>
        {/if}
      {/each}
    </div>
    ...
    {#if tags.length > 0}
      <div class="mt-5">
        <h2 class="mb-3 text-sm font-semibold text-slate-700">Your Keytags</h2>
        <div class="flex flex-wrap justify-center gap-2">
          {#each tags as tag, i (i)}
            <div class="flex flex-col items-center gap-1">
              <img src={tag.image} alt={tag.label} class="h-20 w-auto" />
              <span class="text-[10px] font-medium text-slate-500">{tag.label}</span>
            </div>
          {/each}
        </div>
      </div>
